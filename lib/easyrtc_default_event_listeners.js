 ...
 */
eventListener.onAuthenticate = function (
  socket,
  easyrtcid,
  appName,
  username,
      ...
      if (err) {
        pub.util.logError(
          "[" +
            ...
            "][" +
            easyrtcid +
            "] Unhandled disconnect listener error.",
          err
        );
      }
    });
...
eventListener.onConnection = function (socket, easyrtcid, next) {
  ...
  socket.on("easyrtcAuth", function (msg, socketCallback) {
    ...
    pub.events.emit(
      ...
      msg,
      socketCallback,
      function (err, newConnectionObj) {
        if (err) {
          pub.util.logError(
            "[" + easyrtcid + "] Unhandled easyrtcCmd listener error.",
            err
          );
          return;
        }
 ...
 */
eventListener.onEasyrtcAuth = function (
  socket,
  easyrtcid,
  msg,
  socketCallback,
  ...
  var appName;
  var newAppName =
    _.isObject(msg.msgData) && _.isString(msg.msgData.applicationName)
      ? msg.msgData.applicationName
      : pub.getOption("appDefaultName");
  ...
  if (!_.isFunction(socketCallback)) {
    pub.util.logWarning(
      "[" +
        easyrtcid +
        "] EasyRTC Auth message received with no callback. Disconnecting socket.",
      msg
    );
    try {
      socket.disconnect();
  ...
  if (
    !_.isObject(msg) ||
    !_.isString(msg.msgType) ||
    msg.msgType !== "authenticate"
  ) {
    pub.util.logWarning(
      "[" +
        easyrtcid +
        "] EasyRTC Auth message received without msgType of 'authenticate'. Disconnecting socket.",
      msg
    );
    pub.util.sendSocketCallbackMsg(
      easyrtcid,
      socketCallback,
      pub.util.getErrorMsg("LOGIN_BAD_AUTH"),
      appObj
    );
    try {
      socket.disconnect();
  ...
  if (
    ...
    pub.util.logWarning(
      "[" +
        easyrtcid +
        "] EasyRTC Auth message received with improper msgData. Disconnecting socket.",
      msg
    );
    pub.util.sendSocketCallbackMsg(
      easyrtcid,
      socketCallback,
      pub.util.getErrorMsg("LOGIN_BAD_STRUCTURE"),
      appObj
    );
    try {
      socket.disconnect();
  ...
  async.waterfall(
    [
      function (asyncCallback) {
        ...
        pub.util.isValidIncomingMessage(
          "easyrtcAuth",
          msg,
          null,
          asyncCallback
        ...
        if (!isMsgValid) {
          try {
            pub.util.sendSocketCallbackMsg(
              easyrtcid,
              socketCallback,
              pub.util.getErrorMsg(msgErrorCode),
              appObj
            );
            socket.disconnect();
          } catch (e) {}
          pub.util.logWarning(
            "[" +
              easyrtcid +
              "] EasyRTC Auth message received with invalid message format [" +
              msgErrorCode +
              "]. Disconnecting socket.",
            msg
          );
          callback(
            new pub.util.ConnectionError(
              "[" +
                easyrtcid +
                "] EasyRTC Auth message received with invalid message format [" +
                msgErrorCode +
              ...
              try {
                ...
                pub.util.logInfo(
                  "[" +
                    ...
                    "][" +
                    easyrtcid +
                    "] Authentication denied. Socket disconnected.",
                  err
                );
              } catch (e) {}
            } else {
        ...
        if (isApp) {
          ...
          if (pub.getOption("appAutoCreateEnable")) {
            ...
            pub.util.sendSocketCallbackMsg(
              easyrtcid,
              socketCallback,
              pub.util.getErrorMsg("LOGIN_APP_AUTH_FAIL"),
              appObj
            );
            socket.disconnect();
            pub.util.logWarning(
              "[" +
                easyrtcid +
                "] Authentication failed. Requested application not found [" +
                newAppName +
                "]. Socket disconnected."
            );
          }
      ...
      function (isConnected, asyncCallback) {
        ...
        if (isConnected) {
          ...
          pub.util.logWarning(
            "[" +
              easyrtcid +
              "] Authentication failed. Already connected. Socket disconnected."
          );
          return;
        }
        ...
        if (msg.msgData.roomJoin) {
          for (var currentRoomName in msg.msgData.roomJoin) {
            if (
              !_.isString(currentRoomName) ||
              !appObj.getOption("roomNameRegExp").test(currentRoomName)
            ) {
              pub.events.emit(
                "emitReturnError",
                socketCallback,
                "MSG_REJECT_TARGET_ROOM",
                pub.util.nextToNowhere
              );
              pub.util.logInfo(
                "[" +
                  easyrtcid +
                  "] Authentication failed. Requested room name not allowed [" +
                  currentRoomName +
                  "]."
              );
              return;
      ...
      function (asyncCallback) {
        ...
        if (
          _.isObject(msg.msgData.roomJoin) &&
          !_.isEmpty(msg.msgData.roomJoin)
        ) {
          async.each(
            Object.keys(msg.msgData.roomJoin),
            function (currentRoomName, roomCallback) {
              appObj.isRoom(currentRoomName, function (err, isRoom) {
                ...
                if (
                  msg.msgData.roomJoin[currentRoomName] &&
                  _.isObject(
                    msg.msgData.roomJoin[currentRoomName].roomParameter
                  )
                ) {
                  currentRoomParameter =
                    msg.msgData.roomJoin[currentRoomName].roomParameter;
                }
                ...
                if (isRoom) {
                  ...
                  try {
                    pub.util.sendSocketCallbackMsg(
                      easyrtcid,
                      socketCallback,
                      pub.util.getErrorMsg("LOGIN_BAD_ROOM"),
                      appObj
                    );
                    socket.disconnect();
                  } catch (e) {}
                  pub.util.logInfo(
                    "[" +
                      easyrtcid +
                      "] Authentication failed. Requested room name does not exist [" +
                      currentRoomName +
                      "]."
                  );
                }
        ...
        // If no room is initially provided, have them join the default room (if enabled)
          ...
          pub.events.emit(
            "roomJoin",
            connectionObj,
            connectionObj.getApp().getOption("roomDefaultName"),
            null,
            function (err, roomObj) {
              asyncCallback(err);
            }
              ...
              if (err) {
                pub.util.logError(
                  "[" +
                    ...
                    "][" +
                    easyrtcid +
                    "] Unhandled easyrtcCmd listener error.",
                  err
                );
              }
            }
              ...
              if (err) {
                pub.util.logError(
                  "[" +
                    ...
                    "][" +
                    easyrtcid +
                    "] Unhandled easyrtcMsg listener error.",
                  err
                );
              }
            }
      ...
      if (err) {
        try {
          pub.util.sendSocketCallbackMsg(
            easyrtcid,
            socketCallback,
            pub.util.getErrorMsg("LOGIN_GEN_FAIL"),
            appObj
          );
          socket.disconnect();
          pub.util.logError(
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEasyrtcCmd = function (
  connectionObj,
  msg,
  socketCallback,
  next
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] EasyRTC command received with msgType [" +
      msg.msgType +
      "]"
  );
  ...
  if (!_.isFunction(socketCallback)) {
    pub.util.logWarning(
      "[" +
        ...
        "][" +
        connectionObj.getEasyrtcid() +
        "] EasyRTC command message received with no callback. Ignoring.",
      msg
    );
    return;
  }
      ...
      function (asyncCallback) {
        ...
        switch (msg.msgType) {
          ...
          case "setRoomApiField":
            pub.events.emit(
              "msgTypeSetRoomApiField",
              connectionObj,
              msg.msgData.setRoomApiField,
              socketCallback,
              next
            );
            break;
          ...
          case "hangup":
            ...
            connectionObj
              ...
              .connection(
                msg.targetEasyrtcid,
                function (err, targetConnectionObj) {
                  if (err) {
                    pub.util.sendSocketCallbackMsg(
                      easyrtcid,
                      socketCallback,
                      pub.util.getErrorMsg("MSG_REJECT_TARGET_EASYRTCID"),
                      appObj
                    );
                    pub.util.logWarning(
                      "[" +
                        ...
                        "][" +
                        connectionObj.getEasyrtcid() +
                        "] Could not send WebRTC signal to client [" +
                        msg.targetEasyrtcid +
                        "]. They may no longer be online."
                    );
                    return;
                  }
                  pub.events.emit(
                  ...
                  pub.util.sendSocketCallbackAck(
                    easyrtcid,
                    socketCallback,
                    appObj
                  );
          ...
          default:
            pub.util.sendSocketCallbackMsg(
              easyrtcid,
              socketCallback,
              pub.util.getErrorMsg("MSG_REJECT_BAD_TYPE"),
              appObj
            );
            pub.util.logWarning(
              "[" +
                ...
                "][" +
                connectionObj.getEasyrtcid() +
                "] Received easyrtcCmd message with unhandled msgType.",
              msg
            );
            next(null);
        }
      ...
      if (err) {
        try {
          pub.util.sendSocketCallbackMsg(
            easyrtcid,
            socketCallback,
            pub.util.getErrorMsg("MSG_REJECT_GEN_FAIL"),
            appObj
          );
          pub.util.logWarning(
            "[" +
              ...
              "][" +
              connectionObj.getEasyrtcid() +
              "] Received easyrtcCmd message with general error.",
            msg
          );
        } catch (e) {}
      }
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEasyrtcMsg = function (
  connectionObj,
  msg,
  socketCallback,
  next
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] EasyRTC message received of type [" +
      msg.msgType +
      "]"
  );
  ...
  if (!_.isFunction(socketCallback)) {
    pub.util.logWarning(
      "[" +
        ...
        "][" +
        connectionObj.getEasyrtcid() +
        "] EasyRTC message received with no callback. Ignoring message.",
      msg
    );
    return;
  }
  ...
  async.waterfall(
    [
        ...
        if (!isMsgValid) {
          ...
          pub.util.logWarning(
            "[" +
              ...
              "][" +
              connectionObj.getEasyrtcid() +
              "] EasyRTC message received with invalid message format [" +
              msgErrorCode +
              "].",
            msg
          );
      ...
      function (asyncCallback) {
        ...
        if (
          msg.targetEasyrtcid !== undefined &&
          msg.targetEasyrtcid === connectionObj.getEasyrtcid()
        ) {
          pub.util.sendSocketCallbackMsg(
            easyrtcid,
            socketCallback,
          ...
          pub.util.logWarning(
            "[" +
              ...
              "][" +
              connectionObj.getEasyrtcid() +
              "] EasyRTC message received with improper targetEasyrtcid",
            msg
          );
          return;
        }
        ...
        if (msg.targetEasyrtcid !== undefined) {
          ...
          async.waterfall(
            [
              function (asyncCallback) {
                ...
                connectionObj
                  .getApp()
                  .connection(msg.targetEasyrtcid, asyncCallback);
              },
                    ...
                    function (err, isAllowed) {
                      if (err || !isAllowed) {
                        pub.util.sendSocketCallbackMsg(
                          easyrtcid,
                          socketCallback,
                          pub.util.getErrorMsg("MSG_REJECT_TARGET_ROOM"),
                          appObj
                        );
                        pub.util.logWarning(
                          "[" +
                            ...
                            "][" +
                            connectionObj.getEasyrtcid() +
                            "] EasyRTC message received with improper target room",
                          msg
                        );
                        return;
                      }
                    ...
                    function (err, isAllowed) {
                      if (err || !isAllowed) {
                        pub.util.sendSocketCallbackMsg(
                          easyrtcid,
                          socketCallback,
                          pub.util.getErrorMsg("MSG_REJECT_TARGET_GROUP"),
                          appObj
                        );
                        pub.util.logWarning(
                          "[" +
                            ...
                            "][" +
                            connectionObj.getEasyrtcid() +
                            "] EasyRTC message received with improper target group",
                          msg
                        );
                        return;
                      }
            ...
            function (err) {
              if (err) {
                ...
                pub.util.logError(
                  "[" +
                    connectionObj.getEasyrtcid() +
                    "] General message error. Message ignored.",
                  err
                );
              } else {
                pub.util.sendSocketCallbackAck(
            ...
            [
              ...
              function (connectedEasyrtcidArray, asyncCallback) {
                for (var i = 0; i < connectedEasyrtcidArray.length; i++) {
                  ...
                  if (
                    connectedEasyrtcidArray[i] === connectionObj.getEasyrtcid()
                  ) {
                    continue;
                  }
          ...
          pub.util.logWarning(
            "[" +
              ...
              "][" +
              connectionObj.getEasyrtcid() +
              "] EasyRTC message received without targetEasyrtcid or targetRoom",
            msg
          );
          next(null);
        }
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEmitEasyrtcCmd = function (
  connectionObj,
  msgType,
  msg,
  socketCallback,
  ...
  if (!_.isObject(connectionObj)) {
    next(
      new pub.util.ConnectionError(
        "Connection object invalid. Client may have disconnected."
      )
    );
    return;
  }
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onEmitEasyrtcCmd' with msgType [" +
      msgType +
      "]"
  );
  if (!msg) {
      ...
      if (
        ...
        pub.util.logDebug(
          "[" +
            ...
            "][" +
            connectionObj.getEasyrtcid() +
            "] EasyRTC message: unhandled Ack return message."
        );
      } else {
        pub.util.logDebug(
          "[" +
            ...
            "][" +
            connectionObj.getEasyrtcid() +
            "] EasyRTC message: unhandled return message.",
          returnMsg
        );
      }
    };
  ...
  if (pub.getOption("logMessagesEnable")) {
    try {
      ...
      pub.util.logDebug(
        "[" + appName + "][" + easyrtcid + "] Sending socket.io message"
      );
    }
  }
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEmitEasyrtcMsg = function (
  connectionObj,
  msgType,
  msg,
  socketCallback,
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onEmitEasyrtcMsg' with msgType [" +
      msgType +
      "]"
  );
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEmitError = function (
  connectionObj,
  errorCode,
  socketCallback,
  next
    ...
    socketCallback = function (returnMsg) {
      pub.util.logDebug(
        "[" +
          ...
          "][" +
          connectionObj.getEasyrtcid() +
          "] EasyRTC info: unhandled ACK return message.",
        returnMsg
      );
    };
  }
  ...
  pub.events.emit(
    ...
    connectionObj,
    "error",
    pub.util.getErrorMsg(errorCode),
    socketCallback,
    next
  );
};
  ...
  if (!_.isFunction(socketCallback)) {
    pub.util.logWarning(
      "[" +
        ...
        "][" +
        connectionObj.getEasyrtcid() +
        "] EasyRTC: unable to return ack to socket."
    );
    return;
  }
  if (!_.isFunction(next)) {
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEmitReturnError = function (
  connectionObj,
  socketCallback,
  errorCode,
  next
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onEmitReturnError'"
  );
  if (!_.isFunction(socketCallback)) {
    pub.util.logWarning(
      "[" +
        ...
        "][" +
        connectionObj.getEasyrtcid() +
        "] EasyRTC: unable to return error to socket. Error code was [" +
        errorCode +
        "]"
    );
    ...
    next(
      new pub.util.ConnectionError(
        "[" +
          ...
          "][" +
          connectionObj.getEasyrtcid() +
          "] Unable to return error to socket. Error code was [" +
          errorCode +
          "]"
      )
    );
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onEmitReturnToken = function (
  connectionObj,
  socketCallback,
  next
) {
  ...
  if (!_.isFunction(socketCallback)) {
    pub.util.logWarning(
      "[" +
        ...
        "][" +
        connectionObj.getEasyrtcid() +
        "] EasyRTC onSendToken called with no socketCallback."
    );
    try {
  ...
  async.waterfall(
    [
      ...
      function (iceServers, asyncCallback) {
        ...
        tokenMsg.msgData.stillAliveInterval = pub.getOption(
          "stillAliveInterval"
        );
...
eventListener.onLog = function (level, logText, logFields, next) {
  ...
  if (logFields !== undefined || logFields !== null) {
    if (pub.getOption("logErrorStackEnable") && pub.util.isError(logFields)) {
      ...
      pub.getOption("logWarningStackEnable") &&
      pub.util.isWarning(logFields)
    ) {
      console.log(
        consoleText,
        pub.getOption("logColorEnable")
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onMsgTypeRoomJoin = function (
  connectionObj,
  rooms,
  socketCallback,
  next
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onMsgTypeRoomJoin'"
  );
  if (!_.isFunction(socketCallback)) {
    pub.util.logWarning(
      "[" +
        ...
        "][" +
        connectionObj.getEasyrtcid() +
        "] EasyRTC info: unhandled socket message callback."
    );
    return;
  }
  ...
  for (var currentRoomName in rooms) {
    if (rooms.hasOwnProperty(currentRoomName)) {
      if (
        ...
        !connectionObj
          .getApp()
          .getOption("roomNameRegExp")
          .test(currentRoomName)
      ) {
        pub.events.emit(
          "emitReturnError",
          socketCallback,
    ...
    function (currentRoomName, roomCallback) {
      appObj.isRoom(currentRoomName, function (err, isRoom) {
        ...
        if (
          ...
          _.isObject(rooms[currentRoomName].roomParameter)
        ) {
        ...
        if (isRoom) {
          ...
          pub.util.logWarning(
            "[" +
              ...
              "][" +
              currentRoomName +
              "] Unable to join non-existent room."
          );
          roomCallback(
            new pub.util.ConnectionError(
              "[" +
    ...
    function (err, newRoomObj) {
      if (err) {
        pub.util.sendSocketCallbackMsg(
          easyrtcid,
          socketCallback,
          pub.util.getErrorMsg("MSG_REJECT_BAD_ROOM"),
          appObj
        );
      ...
      connectionObj.generateRoomClientList(
        "join",
        rooms,
        function (err, roomData) {
          if (err) {
            ...
            pub.util.sendSocketCallbackMsg(
              easyrtcid,
              socketCallback,
              { msgType: "roomData", msgData: { roomData: roomData } },
              appObj
            );
          }
        }
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onMsgTypeRoomLeave = function (
  connectionObj,
  rooms,
  socketCallback,
  next
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onMsgTypeRoomLeave' with rooms: ",
    rooms
  );
  if (!_.isFunction(socketCallback)) {
    socketCallback = function (returnMsg) {
  ...
  async.each(
    Object.keys(rooms),
    function (currentRoomName, asyncCallback) {
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onMsgTypeGetIceConfig = function (
  connectionObj,
  socketCallback,
  next
) {
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onMsgTypeGetIceConfig'"
  );
    ...
    function (err, iceConfigObj) {
      if (err) {
        ...
        pub.util.sendSocketCallbackMsg(
          ...
          {
            msgType: "iceConfig",
            msgData: { iceConfig: { iceServers: iceConfigObj } },
          },
          appObj
        );
      }
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onMsgTypeGetRoomList = function (
  connectionObj,
  socketCallback,
  next
) {
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onMsgTypeGetRoomList'"
  );
  ...
  connectionObj.generateRoomList(function (err, roomList) {
    if (err) {
      pub.util.sendSocketCallbackMsg(
        easyrtcid,
        socketCallback,
        pub.util.getErrorMsg("MSG_REJECT_NO_ROOM_LIST"),
        appObj
      );
    } else {
      pub.util.sendSocketCallbackMsg(
        easyrtcid,
        socketCallback,
        { msgType: "roomList", msgData: { roomList: roomList } },
        appObj
      );
    }
    next(null);
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onMsgTypeSetPresence = function (
  connectionObj,
  presenceObj,
  socketCallback,
  next
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onMsgTypeSetPresence' with setPresence: ",
    presenceObj
  );
  if (!_.isFunction(socketCallback)) {
    socketCallback = function (returnMsg) {
  ...
  connectionObj.setPresence(presenceObj, function (err) {
    if (err) {
      pub.util.sendSocketCallbackMsg(
        easyrtcid,
        socketCallback,
        pub.util.getErrorMsg("MSG_REJECT_PRESENCE"),
        appObj
      );
    } else {
      connectionObj.emitRoomDataDelta(false, function (err, roomDataObj) {
        if (err) {
          ...
          pub.util.sendSocketCallbackMsg(
            easyrtcid,
            socketCallback,
            { msgType: "roomData", msgData: { roomData: roomDataObj } },
            appObj
          );
        }
      });
 ...
 * @param       {nextCallback} next     A success callback of form next(err).
 */
eventListener.onMsgTypeSetRoomApiField = function (
  connectionObj,
  roomApiFieldObj,
  socketCallback,
  next
  ...
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onMsgTypeSetRoomApiField' with apiFieldObj: ",
    roomApiFieldObj
  );
  if (!_.isFunction(socketCallback)) {
    socketCallback = function (returnMsg) {
  ...
  connectionObj.room(
    roomApiFieldObj.roomName,
    function (err, connectionRoomObj) {
      if (err) {
        pub.util.sendSocketCallbackMsg(
          easyrtcid,
          socketCallback,
      ...
      connectionRoomObj.setApiField(roomApiFieldObj.field, function (err) {
        if (err) {
          pub.util.sendSocketCallbackMsg(
            easyrtcid,
            socketCallback,
            pub.util.getErrorMsg("MSG_REJECT_BAD_FIELD"),
            appObj
          );
          next(null);
          return;
        }
        connectionRoomObj.emitRoomDataDelta(
          false,
          function (err, roomDataDelta) {
            if (err) {
              pub.util.sendSocketCallbackMsg(
                easyrtcid,
                socketCallback,
 ...
 */
eventListener.onRoomCreate = function (
  appObj,
  ...
  pub.util.logDebug(
    "[" +
      ...
      "]" +
      (creatorConnectionObj
        ? "[" + creatorConnectionObj.getEasyrtcid() + "]"
        : "") +
      " Room [" +
      roomName +
      "] Running func 'onRoomCreate'"
  );
 ...
 */
eventListener.onRoomJoin = function (
  connectionObj,
  roomName,
  roomParameter,
  callback
  ...
  if (_.isFunction(roomParameter)) {
    pub.util.logWarning(
      "Upgrade notice: EasyRTC roomJoin event called without roomParameter object."
    );
...
eventListener.onRoomLeave = function (connectionObj, roomName, next) {
  pub.util.logDebug(
    "[" +
      ...
      "][" +
      connectionObj.getEasyrtcid() +
      "] Running func 'onRoomLeave' with rooms [" +
      roomName +
      "]"
  );
    ...
    [
      function (callback) {
        ...
        if (pub.getOption("demosEnable")) {
          pub.util.logDebug(
            "Setting up demos to be accessed from '" +
              pub.getOption("demosPublicFolder") +
              "/'"
          );
          pub.httpApp.get(
            pub.getOption("demosPublicFolder") + "/*",
        ...
        if (pub.getOption("apiEnable")) {
          ...
          pub.util.logDebug(
            "Setting up API files to be accessed from '" +
              pub.getOption("apiPublicFolder") +
              "/'"
          );
          pub.httpApp.get(
            pub.getOption("apiPublicFolder") + "/easyrtc.js",
            function (req, res) {
          ...
          pub.httpApp.get(
            pub.getOption("apiPublicFolder") + "/easyrtc_ft.js",
            function (req, res) {
              pub.util.sendSessionCookie(req, res);
          ...
          pub.httpApp.get(
            pub.getOption("apiPublicFolder") + "/easyrtc.css",
            function (req, res) {
              pub.util.sendSessionCookie(req, res);
          ...
          pub.httpApp.get(
            pub.getOption("apiPublicFolder") + "/easyrtc.min.js",
            function (req, res) {
              pub.util.sendSessionCookie(req, res);
              (res.sendFile || res.sendfile).call(
                res,
                "open_source/api/easyrtc.min.js",
                { root: __dirname + "/../" }
              );
            }
          );
          pub.httpApp.get(
            pub.getOption("apiPublicFolder") + "/easyrtc_ft.min.js",
            function (req, res) {
              pub.util.sendSessionCookie(req, res);
              (res.sendFile || res.sendfile).call(
                res,
                "api/easyrtc_ft.min.js",
                { root: __dirname + "/../" }
              );
            }
          );
          pub.httpApp.get(
            pub.getOption("apiPublicFolder") + "/easyrtc.min.css",
            function (req, res) {
              pub.util.sendSessionCookie(req, res);
              (res.sendFile || res.sendfile).call(
                res,
                "open_source/api/easyrtc.min.css",
                { root: __dirname + "/../" }
              );
            }
          );
          pub.httpApp.get(
            pub.getOption("apiPublicFolder") + "/img/*",
            function (req, res) {
              pub.util.sendSessionCookie(req, res);
              (res.sendFile || res.sendfile).call(
                res,
          ...
          if (pub.getOption("apiLabsEnable")) {
            pub.httpApp.get(
              pub.getOption("apiPublicFolder") + "/labs/*",
              function (req, res) {
                pub.util.sendSessionCookie(req, res);
                (res.sendFile || res.sendfile).call(
                  res,
        ...
        if (
          pub.getOption("apiEnable") &&
          pub.getOption("apiOldLocationEnable")
        ) {
          pub.util.logWarning(
            "Enabling listening for API files in older depreciated location."
          );
      ...
      function (callback) {
        ...
        pub.socketServer.sockets.on("connection", function (socket) {
          ...
          pub.util.logDebug(
            "[" + easyrtcid + "][" + socket.id + "] Socket connected"
          );
